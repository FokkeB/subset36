...
// fills the highest part of the return values with 0's if the last word is requested
{
    ...
    if ( (bit_index > 0) && (word_index < WORDS_IN_LONGNUM) )
...
// uses v as verbosity level
{
    ...
    for (j=(long_get_order(longnum)-1)/BITS_IN_WORD; j>=0; j--)
    {
        printf ("#%02d=", j);
...
// write per byte (instead of per whole word) to prevent unwanted leading 0's
{
    ...
    for (j = (n - 1) / 8; j >= 0; j--)
        printf("%02X", long_get_word(longnum, j * 8) & 0xFF);
}
...
void long_print_fancy(int v, t_longnum longnum, int wordlength, int size, t_longnum_layout *longnum_layout)
/**
