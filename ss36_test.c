...
#define OPT_VERSION "3.19"          // needed to satisfy OPT-library
...
// Otherwise output to a csv-file with name output_to
{
    ...
    {
        align_telegram(p_telegram, a_enc);
        i = sprint_longnum_hex(line, p_telegram->deshaped_contents);
        line[i] = ',';
        line[i + 1] = 0;
 
        if ( (format != NULL) && (strcmp(format, "hex") == 0) )
            sprint_longnum_hex(&line[i+1], p_telegram->contents);
        else
        {
