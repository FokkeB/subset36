...
telegram* parse_input_line(const char* line_orig, const bool force_long)
...
// todo: make string out of line?
{
    ...
    p_telegram->force_long = force_long;
...
telegram* parse_content_string(const string& contents_orig, const bool force_long)
// parses the balise information in contents into a linked list
// sets the force_long-parameter
// returns a pointer to the first telegram in the list
{
    ...
    telegram* current_telegram = NULL;   // current (=last) telegram
    telegram* telegramlist = NULL;       // pointer to start of list
        ...
        // iterate over the lines (separated by LINE_DELIM in contents
    {
        ...
        if ((p_new_telegram = parse_input_line(line.c_str(), force_long)) == NULL)
        ...
        {
            if (!telegramlist)
                // first telegram, start the list and remember the first
                telegramlist = p_new_telegram;
            else
                // add line to list of telegrams:
                current_telegram->next = p_new_telegram;

            current_telegram = p_new_telegram;
